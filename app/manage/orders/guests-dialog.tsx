...
import { useGuestList } from '@/queries/useAccount'
...
export default function GuestsDialog({ onChoose }: { onChoose: (guest: GuestItem) => void }) {
  ...
  const guestList = useGuestList({fromDate, toDate})
  const data = guestList.data?.payload.data ?? []
            ...
            <div className='flex items-center justify-end space-x-2 py-4'>
              ...
              <div>
                <AutoPagination
                  ...
                  isLink={false}
                  onClick={pageNumber => {
                    table.setPagination({ pageIndex: pageNumber - 1 , pageSize: PAGE_SIZE })
                  }}
                />
              </div>
            </div>
