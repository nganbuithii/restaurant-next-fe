...
const request = async (
    ...
    const baseUrl =
        options?.baseUrl === undefined
            ? envConfig.NEXT_PUBLIC_RESTAURANT_API
            : options.baseUrl
    ...
    let payload: { message: string; [key: string]: unknown };
    try {
        const json = await res.json();
        payload = typeof json === "object" && json !== null && "message" in json
            ? (json as { message: string; [key: string]: unknown })
    ...
    if (!res.ok) {
        if (res.status === ENTITY_ERROR_STATUS) {
            ...
            if (isClient) {
                ...
                const accessToken = (options?.headers as Record<string, string>)?.Authorization.split(
                    'Bearer '
                )[1]
